...
async def _call_next_endpoint(client: httpx.AsyncClient, endpoint: str) -> httpx.Response:
    if endpoint == "bulk":
...
def _looks_blocked(resp: httpx.Response) -> str | None:
    ...
    if resp.status_code in (500, 502, 503, 504):
        return f"HTTP {resp.status_code} (WAF/yük dengeleyici bu kodlarla da engelleyebilir)"
    content_type = resp.headers.get("content-type", "")
    if "json" not in content_type and resp.status_code == 200:
        return f"beklenmeyen content-type: {content_type!r} (CAPTCHA/HTML sayfası olabilir)"
    if resp.headers.get("retry-after"):
        return f"Retry-After header görüldü: {resp.headers['retry-after']}"
    return None


async def run_ramp_stage(client: httpx.AsyncClient, requests_per_sec: int, total_requests: int, cycle: itertools.cycle) -> tuple[int, str | None]:
    ...
    """
    print(f"\n=== Kademe: hedef {requests_per_sec} istek/sn, toplam {total_requests} istek ===")
    ...
    for batch_idx in range(num_batches):
        ...
        for endpoint_name, result in zip(endpoint_names, results):
            ...
            if isinstance(result, httpx.RequestError):
                print(f"  [batch {batch_idx + 1}] ENGELLENDİ (ağ seviyesi): {endpoint_name} isteğinde {type(result).__name__}: {result}")
                return completed, f"ağ seviyesi hata ({type(result).__name__}) — muhtemel IP engeli (bağlantı reddedildi/zaman aşımına uğradı)"
            if isinstance(result, BaseException):
                raise result
            block_reason = _looks_blocked(result)
            if block_reason:
                print(f"  [batch {batch_idx + 1}] ENGELLENDİ ({endpoint_name}): {block_reason}")
    ...
    print(f"  {completed}/{total_requests} istek tamamlandı, engelleme yok — fiili hız: {actual_rps:.1f} istek/sn (hedef: {requests_per_sec})")
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=httpx.Timeout(10.0, connect=5.0)) as client:
        for stage_index, (requests_per_sec, total_requests) in enumerate(RAMP_STAGES):
            if stage_index > 0:
                print(f"\n... kademeler arası soğuma: {COOLDOWN_BETWEEN_STAGES_SEC}s bekleniyor ...")
                await asyncio.sleep(COOLDOWN_BETWEEN_STAGES_SEC)
            completed, block_reason = await run_ramp_stage(client, requests_per_sec, total_requests, endpoint_cycle)
            if block_reason:
                print(f"\n>>> DURDURULDU: hedef {requests_per_sec} istek/sn kademesinde, {completed}. istekte engellendi.")
                print(f">>> Sebep: {block_reason}")
                return
    print("\n>>> Tüm kademeler (1, 5, 10 istek/sn) engelleme görülmeden tamamlandı.")
