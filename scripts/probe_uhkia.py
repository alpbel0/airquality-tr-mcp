"""Faz 1.3 — manuel keşif scripti: sim.csb.gov.tr'ye tarayıcısız (headless) erişimi doğrular.
...
async def test_bulk_stations(client: httpx.AsyncClient) -> None:
    ...
    sample = stations[0]
    print(f"örnek istasyon: {sample['Name']} ({sample['City_Title']}), AQI={sample['Values']['AQIIndex']}")
...
async def test_station_detail(client: httpx.AsyncClient) -> None:
    ...
    print(f"kayıt sayısı: {len(values)} (ilk: {values[0]['Date']}, son: {values[-1]['Date']})")
...
async def test_category_and_params(client: httpx.AsyncClient) -> None:
    ...
    resp = await client.post("/Services/GetAirQualityStatus?type=0", headers=COMMON_HEADERS)
    categories = resp.json()["objects"]
    for c in categories:
        print(f"  {c['Value']:>10} ({c['Min']}-{c['Max']}): {c['Color']}")

    print("\n=== GetAirQualityParams (breakpoint tablosu) ===")
    resp = await client.post("/Services/GetAirQualityParams?type=0", headers=COMMON_HEADERS)
    params = resp.json()["objects"]
    for pollutant, breakpoints in params.items():
        print(f"  {pollutant}: {len(breakpoints)} breakpoint" + (" (BOŞ)" if not breakpoints else ""))
...
async def test_error_scenarios(client: httpx.AsyncClient) -> None:
    ...
    if resp.status_code in (301, 302, 303, 307, 308):
        ...
        follow = await client.get(resp.headers["location"], headers=COMMON_HEADERS)
        print("  → redirect hedefi status:", follow.status_code, "| content-type:", follow.headers.get("content-type"))
    is_json = "json" in resp.headers.get("content-type", "")
    print("JSON mu?", is_json, "-> server kodumuz bunu 'beklenmeyen upstream yaniti' olarak sarmalamali")

    print("\n=== Hata senaryosu: zaman aşımı (kısa timeout ile) ===")
    try:
        async with httpx.AsyncClient(base_url=BASE_URL, timeout=httpx.Timeout(0.001, connect=5.0)) as short_client:
        ...
        print("beklenen ReadTimeout hatası doğru şekilde fırlatıldı (httpx.Timeout mekanizması çalışıyor)")
